...
require realpath(dirname(__DIR__) . '/includes/config.php');   // Utilisation de realpath pour les chemins corrects
require realpath(dirname(__DIR__) . '/includes/functions.php'); // Utilisation de realpath pour les chemins corrects

if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity']) > $timeout_duration) {
    session_unset();
    session_destroy();
    header("Location: {$config['base_url']}users/login.php?timeout=1");
    exit();
}
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $config['base_url']; ?>css/common.css">
    <link rel="stylesheet" href="<?php echo $config['base_url']; ?>css/shoutbox.css">
...
<body>
    ...
    <?php if ($is_admin): ?>
        <a href="<?php echo $config['base_url']; ?>admin.php" class="admin-button">Administration</a>
    <?php elseif ($is_moderator): ?>
        <a href="<?php echo $config['base_url']; ?>admin.php" class="moderation-button">Modération</a>
    <?php endif; ?>
        ...
        <div class="messages" id="messages">
            ...
            if (is_array($messages) && !empty($messages)) {
                foreach ($messages as $message):
                    ...
                    $user_class = $role === 'admin' ? 'admin' : ($role === 'moderator' ? 'moderator' : 'user');
        ...
        <div class="emoji-container" id="emoji-container" style="display: none;">
            ...
            if (is_array($emoji_list) && !empty($emoji_list)) {
                foreach ($emoji_list as $emoji): ?>
                    <img src="<?php echo $config['base_url']; ?>assets/emojis/<?php echo $emoji; ?>.svg" class="emoji" alt="<?php echo $emoji; ?>" />
                <?php endforeach;
            } else {
    ...
    <audio id="newMessageSound" src="<?php echo $config['base_url']; ?>assets/sounds/notification.mp3"></audio>
    <audio id="myMessageSound" src="<?php echo $config['base_url']; ?>assets/sounds/my_message.mp3"></audio>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.emoji').forEach(function(emoji) {
                emoji.addEventListener('click', function() {
                    const imgTag = `<img src="<?php echo $config['base_url']; ?>assets/emojis/${emoji.alt}.svg" alt="${emoji.alt}" class="emoji-inline">`;
